// FCM Tokens management for NRD Data Access Admin
...
function showError(message) {
  if (typeof logger !== 'undefined' && logger.error) {
    logger.error('FCM Tokens Error', { message });
  } else {
    console.error('FCM Tokens Error:', message);
  }
  alert('Error: ' + message);
}
...
function showSuccess(message) {
  if (typeof logger !== 'undefined' && logger.info) {
    logger.info('FCM Tokens Success', { message });
  } else {
    console.log('FCM Tokens Success:', message);
  }
  alert(message);
}
...
function loadFCMTokens() {
  ...
  if (!nrd.fcmTokens) {
    const errorMsg = 'FCMTokensService not available. Please ensure you are using the latest version of nrd-data-access library.';
    if (typeof logger !== 'undefined' && logger.error) {
  ...
  if (!tokensList) {
    if (typeof logger !== 'undefined' && logger.warn) {
      ...
      console.warn('FCM tokens list element not found');
    }
    return;
  }
  ...
  fcmTokensListener = nrd.fcmTokens.onValue((tokens) => {
    ...
    if (typeof logger !== 'undefined' && logger.debug) {
      logger.debug('FCM tokens data received', { count });
    }
    ...
    (async () => {
      ...
      if (!nrd || !nrd.fcmTokens) {
        if (typeof logger !== 'undefined' && logger.error) {
          ...
          console.error('FCMTokensService not available');
        }
        return;
      }
...
async function deleteFCMTokenHandler(tokenId) {
  ...
  if (!nrd.fcmTokens) {
    const errorMsg = 'FCMTokensService not available';
    if (typeof logger !== 'undefined' && logger.error) {
      logger.error(errorMsg);
    } else {
      console.error(errorMsg);
...
// Load FCM tokens when tab is shown
function loadFCMTokensTab() {
  loadFCMTokens();
}

// Export functions for global use
window.initializeFCMTokens = initializeFCMTokens;
window.loadFCMTokensTab = loadFCMTokensTab;
window.cleanupFCMTokens = cleanupFCMTokens;
